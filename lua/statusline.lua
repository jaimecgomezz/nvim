...
M.run = function()
    ...
    local function fileInfo()
        ...
        if filename ~= "Empty " then
            local devicons_present, devicons = pcall(require,
                                                     "nvim-web-devicons")
    ...
    local function gitchanges()
        if not vim.b.gitsigns_head or vim.b.gitsigns_git_status or vim.o.columns <
            120 then return "" end
    ...
    local function LSP_Diagnostics()
        if not rawget(vim, "lsp") then return " 󰅚 0  0" end
        ...
        errors = (errors and errors > 0) and ("󰅚 " .. errors .. " ") or
                     "󰅚 0 "
        warnings =
            (warnings and warnings > 0) and (" " .. warnings .. " ") or
                " 0 "
    ...
    local function LSP_status()
        if rawget(vim, "lsp") then
            for _, client in ipairs(vim.lsp.get_active_clients()) do
                if client.attached_buffers[vim.api.nvim_get_current_buf()] and
                    client.name ~= "null-ls" then
                    return (vim.o.columns > 100 and " 󰄭  " .. client.name ..
                               "  ") or " 󰄭  LSP  "
                end
            end
        end
    ...
    local function file_encoding()
        return
            string.upper(vim.bo.fileencoding) == "" and "" or "%#St_encode#" ..
                string.upper(vim.bo.fileencoding) .. "  "
    end

    local function cwd()
        local dir_name =
            "%#St_Mode# 󰉖 " .. fn.fnamemodify(fn.getcwd(), ":t") .. " "
        return (vim.o.columns > 85 and dir_name) or ""
    end

    return table.concat {
        mode(), fileInfo(), git(), LSP_Diagnostics(), "%=", LSP_progress(),
        "%=", gitchanges(), cursor_position(), file_encoding(), filetype(),
        LSP_status() or "", cwd()
    }
end
